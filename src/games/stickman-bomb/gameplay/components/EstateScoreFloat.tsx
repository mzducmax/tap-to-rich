  ...
  return (
    <motion.span
      initial={{
        ...
        opacity: 0,
        scale: presentation.initialScale,
        rotate: isSoccerBall ? -18 : isMole ? -5 : 0,
      ...
      animate={{
        ...
        rotate: isSoccerBall ? [-18, 8, -4, 0] : isMole ? [-5, 5, -2, 0] : 0,
        x: isLightningStrike
          ? ['-50%', '-47%', '-53%', '-49%', '-50%', '-50%']
          : '-50%',
      ...
      style={{
        ...
        textShadow: isLightningStrike
          ...
          : isSoccerBall
            ? `0 0 ${presentation.glowBlur} ${palette.glow}, 0 0 14px rgba(255,255,255,0.85), 0 4px 0 rgba(0,0,0,0.55), 0 8px 18px rgba(0,0,0,0.72)`
            : isMole
        ...
        letterSpacing: source === 'key-9'
          ? '0.04em'
          : isSoccerBall || isLightningStrike
            ? '0.03em'
            : undefined,
        filter: isSoccerBall ? 'drop-shadow(0 0 10px rgba(255,255,255,0.45))' : undefined,
      }}
    >
      {isLightningStrike ? '⚡ ' : isSoccerBall ? '⚽ ' : isMole ? `${MOLE_SCORE_FLOAT_THEME.emoji} ` : ''}
      {sign}
      {formatScoreFloatAmount(amount)}$
    </motion.span>
  );
