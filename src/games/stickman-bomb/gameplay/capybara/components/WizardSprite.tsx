/**
 * Clipped viewport wizard capybara sprite — per-frame translate.
...
function loadBakedSheet(): Promise<string> {
  ...
  if (!bakeSheetPromise) {
    bakeSheetPromise = new Promise((resolve, reject) => {
      ...
      raw.onerror = () => reject(new Error('Failed to load wizard sprite sheet'));
      raw.src = WIZARD_SPRITE_URL;
    });
  }
  return bakeSheetPromise;
}

export const WizardSprite = memo(function WizardSprite({
  frameIndices,
  frameMs = 180,
  startFrame = 0,
  paused = false,
  ...
  useEffect(() => {
    ...
    const applyFrame = () => {
      ...
      const { x, y } = getWizardFrameOffset(frameIndices[seqIndex]);
  ...
  return (
    <div className="wizard-sprite-viewport">
      {sheetSrc ? (
        <img
          ref={sheetRef}
          className="wizard-sprite-sheet"
          src={sheetSrc}
          alt=""
          draggable={false}
        />
